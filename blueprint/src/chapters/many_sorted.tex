...
\begin{definition}[Many-sorted category structure]
  ...
  consists of, for each pair of indices $k, j \in \Index$:
  ...
  \Tg^{(k,j)}, \Idm^{(k,j)}, \circ^{(k,j)})_{k, j \in \Index})$.
\end{definition}
...
\begin{definition}[Pre-many-sorted category]
  ...
  That is, for each pair of indices $k, j \in \Index$ with $j < k$, the data $(C_j, C_k,
...
\begin{definition}[Many-sorted functor]
  ...
  C_k \to D_k)_{k \in \Index}$ satisfying, for each pair of indices $k, j \in \Index$ with $j < k$,
