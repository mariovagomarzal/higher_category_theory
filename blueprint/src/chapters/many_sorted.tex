...
\begin{definition}[Many-sorted category]
  ...
  \begin{enumerate}
    \item \label{ax:ms_cross_glob} For all $f \in C_k$ and all indices $i, j, k \in \Index$ with $i
      < j < k$,
      \begin{align*}
      ...
      \end{align*}
    \item \label{ax:ms_cross_sc_tg_comp} For all $f, g \in C_k$ and all indices $i, j, k \in \Index$
      with $i < j < k$, if $g$ and $f$ are $(k,i)$-composable, then $\Sc^{(k,j)} (g)$ and
      $\Sc^{(k,j)} (f)$ and $\Tg^{(k,j)} (g)$ and $\Tg^{(k,j)} (f)$ are also $(j,i)$-composable, and
      \begin{align*}
        \Sc^{(k,j)} (g \circ^{(k,i)} f) = \Sc^{(k,j)} (g) \circ^{(j,i)} \Sc^{(k,j)} (f), \\
        \Tg^{(k,j)} (g \circ^{(k,i)} f) = \Tg^{(k,j)} (g) \circ^{(j,i)} \Tg^{(k,j)} (f).
      \end{align*}
    \item \label{ax:ms_idm_idm} For all $f \in C_i$ and all indices $i, j, k \in \Index$ with $i < j
      < k$,
      \[
        \Idm^{(k,j)} (\Idm^{(j,i)} (f)) = \Idm^{(k,i)} (f).
      \]
    \item \label{ax:ms_idm_comp} For all $f, g \in C_j$ and all indices $i, j, k \in \Index$ with $i
      < j < k$, if $g$ and $f$ are $(j,i)$-composable, then $\Idm^{(k,j)} (g)$ and $\Idm^{(k,j)}
      (f)$ are $(k,i)$-composable, and
      \[
        \Idm^{(k,j)} (g \circ^{(j,i)} f) = \Idm^{(k,j)} (g) \circ^{(k,i)} \Idm^{(k,j)} (f).
      \]
    \item \label{ax:ms_interchange} For all $f_1, f_2, g_1, g_2 \in C_k$ and all indices $i, j, k
      \in \Index$ with $i < j < k$, suppose that:
      \begin{itemize}
...
\begin{definition}[Many-sorted functor]
  ...
  \leanok
  Let $\cat{C} = ((C_k)_{k \in \Index}, (\Sc^{(k,j)}, \Tg^{(k,j)}, \Idm^{(k,j)}, \circ^{(k,j)})_{j,
  k \in \Index, \, j < k})$ and $\cat{D} = ((D_k)_{k \in \Index}, (\Sc^{(k,j)}, \Tg^{(k,j)},
  \Idm^{(k,j)}, \circ^{(k,j)})_{j, k \in \Index, \, j < k})$ be many-sorted $\Index$-categories. A
  \emph{many-sorted $\Index$-functor} from $\cat{C}$ to $\cat{D}$ is a family of maps $(F_k \colon
  C_k \to D_k)_{k \in \Index}$ satisfying, for each pair of indices $j, k \in \Index$ with $j < k$,
  the following preservation properties:
...
\begin{definition}[Category of many-sorted categories]
  ...
  \begin{enumerate}
    ...
    \item \label{ax:ms_cat_comp} Composition of morphisms is given by composition of many-sorted
      functors, which is well defined by Theorem~\ref{thm:ManySortedFunctor_comp}.
    \item \label{ax:ms_cat_id} The identity morphism for each object is given by the identity
      many-sorted functor, which is well defined by Theorem~\ref{thm:ManySortedFunctor_id}.
  \end{enumerate}
\end{definition}
...
\begin{notation}
  ...
  many-sorted $n$-categories and many-sorted $n$-functors between them, denoted by $n\CatMS$.
\end{notation}
...
\begin{definition}[Category of many-sorted omega categories]
  ...
  \leanok
  The \emph{category of many-sorted $\omega$-categories}, denoted by $\omega\CatMS$, is defined
  analogously to Definition~\ref{def:ManySortedCategory_category}, but using many-sorted
