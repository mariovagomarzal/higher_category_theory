...
\begin{construction}[Underlying single-sorted finite category]
  ...
  \in \NN$ with $m < n$. The \emph{underlying $m$-category} of $\cat{C}$ is the single-sorted
  $m$-category $\cat{C}^{<m}$ defined on the set of $m$-cells $C_m$, with the source, target, and
  composition operations given by the birestrictions of those of $\cat{C}$ to $C_m$.
\end{construction}
...
\begin{construction}[Underlying single-sorted omega category]
  ...
  \notready
  Let $\cat{C} = (C, (\Sc^k, \Tg^k, \Pcomp^k)_{k \in \NN})$ be a single-sorted $\omega$-category,
  and let $m \in \NN$. The \emph{underlying $m$-category} of $\cat{C}$ is the single-sorted
  $m$-category $\cat{C}^{<m}$ defined by the same construction as in
