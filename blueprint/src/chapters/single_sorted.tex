...
\begin{definition}
  ...
  \begin{enumerate}
    \item \label{ax:ss_sc} A unary operation $\Sc^i \colon C \to C$ for each index $i \in \Index$,
      called the \emph{source} at dimension $i$.
    \item \label{ax:ss_tg} A unary operation $\Tg^i \colon C \to C$ for each index $i \in \Index$,
      called the \emph{target} at dimension $i$.
    \item \label{ax:ss_comp} A partial binary operation $\Pcomp^i \colon C \times C \rightharpoonup
      C$ for each index $i \in \Index$, called the \emph{composition} at dimension $i$.
  \end{enumerate}
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item \label{ax:ss_sc_tg_comp} For all $f, g \in C$, if $g$ and $f$ are composable, then
      ...
      \end{align*}
    \item \label{ax:ss_right_id} For all $f \in C$, the morphisms $f$ and $\Sc^i (f)$ are composable
      and
      \[
        f \Pcomp^i \Sc^i (f) = f.
      \]
    \item \label{ax:ss_left_id} For all $f \in C$, the morphisms $\Tg^i (f)$ and $f$ are composable
      and
      \[
        \Tg^i (f) \Pcomp^i f = f.
      \]
    \item \label{ax:ss_assoc} For all $f, g, h \in C$, if $h$ and $g$ are composable, and $g$ and
      $f$ are composable, then $h \Pcomp^i g$ and $f$ and $h$ and $g \Pcomp^i f$ are composable, and
      \[
...
\begin{definition}[Single-sorted category]
  ...
  \begin{enumerate}
    \item \label{ax:ss_cross_glob} For all $f \in C$ and all indices $j, k \in \Index$ with $j < k$,
      \begin{align*}
      ...
      \end{align*}
    \item \label{ax:ss_cross_sc_tg_comp} For all $f, g \in C$ and all indices $j, k \in \Index$ with
      $j < k$, if $g$ and $f$ are composable at dimension $j$, then $\Sc^k (g)$ and $\Sc^k (f)$ and
      $\Tg^k (g)$ and $\Tg^k (f)$ are also composable at dimension $j$, and
      \begin{align*}
        \Sc^k (g \Pcomp^j f) = \Sc^k (g) \Pcomp^j \Sc^k (f), \\
        \Tg^k (g \Pcomp^j f) = \Tg^k (g) \Pcomp^j \Tg^k (f).
      \end{align*}
    \item \label{ax:ss_interchange} For all $f_1, f_2, g_1, g_2 \in C$ and all indices $j, k \in
      \Index$ with $j < k$, suppose that:
      \begin{itemize}
